...
int verify(char** array, char c, int rows, int cols) { //verify if a player won
...
void replace(char** array, int x, char c, int rows) { // replace the dot with A or B
  x=x-1;
...
void print(char** array, int rows, int cols) { // print the grid
  for(int i=0; i<rows; i++) {
    for(int j=0; j<cols; j++) {
      printf("%c ", array[i][j]);
    }
